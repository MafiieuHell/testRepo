...
function applyReduction(ordersArray) {
  for (let i = 0; i < ordersArray.length; i++) {
    ...
    let date = new Date(ordersArray[i].timestamp*1000)
  let year = date.getFullYear();
  let month = date.getMonth();
  if (month === 6 && year === 2020) {
    let newPrice = ordersArray[i].price - (ordersArray[i].price * 20 /100);
   ordersArray[i].price = newPrice.toFixed(2);
  }
}
